...
import {CreateOrEditReviewSelect, FindAllReviewSelect, FindOneReviewSelect} from "./reviews.select";
...
export class ReviewsService {
    ...
    async update(id: number, data: EditReviewDto, user: Request['user']) {
        ...
        if (review.created_by !== user.id && !user.is_admin) throw new HttpException('You are not allowed to update this review', 403);
    ...
    async remove(id: number, user: Request['user']) {
        ...
        if (!review) throw new HttpException('Review not found', 404);
        if (review.created_by !== user.id && !user.is_admin) throw new HttpException('You are not allowed to delete this review', 403);
